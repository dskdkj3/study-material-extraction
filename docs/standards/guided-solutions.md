...
- When using a convergence or classification test, first state what the test measures, the decision cases, and the inconclusive case. For example, before using the ratio test, explain that it looks at the eventual size of `|u_(n+1)/u_n|`, with limit `< 1` convergent, `> 1` divergent, and `= 1` requiring another check.
- When a solution path becomes long, insert a short route map before the computation and prefer concrete term expansion before abstract index changes. For example, when matching a power series to a known logarithm series, write the first few terms of `S(x)` and `xS(x)` before introducing or omitting `m = n + 1`.
- When an entire subproblem is opaque to a beginner, restart from roles and proof target: identify variables versus parameters, the troublesome endpoint or condition, what must be made small or bounded, and the exact shape of the estimate before applying a named criterion.
...
- "比值判别法要讲一下" means a convergence test was named before its local rule was explained; add what quantity the test checks, how each limit case is interpreted, and why endpoints or inconclusive cases still need separate work.
- "路径太长，看着有点晕" means the computation may be locally clear but globally hard to follow; add a route map and reduce abstract notation by expanding representative terms.
- "从第一小问开始就完全看不懂" means the subproblem needs a fresh beginner-facing entry point; explain variable roles, what the proof is trying to control, and why the next estimate is the right target before any theorem or criterion.
